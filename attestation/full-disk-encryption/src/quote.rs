...
pub fn retrieve_quote(report_data: &[u8; 64],mut boot_type:u8) -> Result<Vec<u8>> {
    // 1. tdreport_data
    ...
    if boot_type == 2 {
        println!("Quote_64: {}", base64::encode(&quote_bytes));
    }
    Ok(quote_bytes)
}
